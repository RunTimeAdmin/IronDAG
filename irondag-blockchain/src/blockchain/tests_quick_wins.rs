...
mod tests {
    ...
    fn test_time_locked_transaction_block() {
        ...
        let tx = Transaction::new(from, to, value, fee, nonce).with_execute_at_block(10);
    ...
    fn test_time_locked_transaction_timestamp() {
        ...
        let tx = Transaction::new(from, to, value, fee, nonce).with_execute_at_timestamp(1000);
    ...
    fn test_gasless_transaction_processing() {
        ...
        // Set up balances
        blockchain.set_balance(Address([1u8; 20]), 2000).unwrap(); // Sender has enough for value
        blockchain.set_balance(Address([3u8; 20]), 500).unwrap(); // Sponsor has enough for fee
        blockchain.set_balance(Address([2u8; 20]), 0).unwrap(); // Receiver starts at 0
        ...
        // Process transaction
        futures::executor::block_on(blockchain.process_transaction(&tx)).unwrap();
    ...
    fn test_regular_transaction_processing() {
        ...
        // Set up balances
        blockchain.set_balance(Address([1u8; 20]), 2000).unwrap(); // Sender has enough for value + fee
        blockchain.set_balance(Address([2u8; 20]), 0).unwrap(); // Receiver starts at 0
    ...
    fn test_reputation_calculation() {
        ...
        for _ in 0..10 {
            manager.record_successful_tx(&address, 1000, &Address([2u8; 20]));
        }
    ...
    fn test_reputation_factors() {
        ...
        manager.record_successful_tx(&address, 1000, &Address([2u8; 20]));
        manager.record_successful_tx(&address, 2000, &Address([3u8; 20]));
        manager.record_failed_tx(&address);
