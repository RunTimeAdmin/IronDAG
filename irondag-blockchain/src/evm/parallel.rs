...
//!
//! This module provides parallel transaction execution for the EVM.
//!
//! ## Isolation Model
//!
//! The parallel execution system uses a **dependency-graph based isolation model**:
//!
//! ### Conflict Detection
//! - Transactions are analyzed to extract `ReadSet` and `WriteSet` information
//! - **Address-level conflicts**: Any two transactions accessing the same address conflict
//! - **Storage-level conflicts**: Access to the same contract storage slot causes conflict
//! - Conflicts are detected using `has_read_write_conflict()`, `has_write_write_conflict()`,
//!   and `has_write_read_conflict()` methods
//!
//! ### Execution Strategy
//! - `execute_parallel_async()`: Uses Tokio tasks for true parallel execution
//!   - Independent transactions execute concurrently using `tokio::spawn()`
//!   - Results are collected with `futures::future::join_all()`
//!   - Sets `executed_in_parallel: true` for transactions in parallel batches
//! - `execute_parallel_sync()`: Groups transactions but executes sequentially
//!   - Used when parallel execution is disabled or for single transactions
//!   - Sets `executed_in_parallel: false` (sequential execution)
//!
//! ### State Management
//! - The `executor` callback (type `F`) is shared via `Arc<F>` across tasks
//! - Each task receives a **cloned `Transaction`** (`tx_clone`), not shared state
//! - The actual `EvmTransactionExecutor` is NOT cloned per task; instead, the
//!   caller-provided executor callback handles state access
//! - State merging is the responsibility of the caller's executor implementation
//!
//! ### Thread Safety
//! - The parallel executor itself is `Send + Sync` (contains only config fields)
//! - Transaction execution happens in spawned Tokio tasks
//! - The executor callback must be `Send + Sync + 'static` for async execution
//! - Results are aggregated after all tasks complete (no shared mutable state)
//!
//! ### Batch Processing
//! - Transactions are grouped into parallel batches using `group_parallel_batches()`
//! - Each batch contains transactions with no inter-dependencies
//! - Batches execute sequentially, but transactions within a batch execute in parallel
    ...
    fn analyze_transaction(&self, tx: &Transaction) -> Result<TransactionDependency, String> {
        ...
        if !tx.data.is_empty() {
            ...
            if tx.to == Address::zero() {
                ...
                // Conservative conflict detection: any calls to the same contract address
                // are treated as potentially conflicting. This avoids false negatives from
                // incorrect storage key extraction. A future ABI-aware implementation can
                // narrow this to actual storage slot conflicts.
                // The contract address (tx.to) is already in write_set.addresses above,
                // so conflicts will be detected by address overlap.
            }
        }
                ...
                if let Some(tx) = graph.get_transaction(tx_hash) {
                    match executor(tx) {
                        Ok(result) => all_results.push(ParallelExecutionResult {
                            ...
                            gas_used: result.gas_used,
                            output: result.output,
                            executed_in_parallel: false, // Sync path executes sequentially; only async path sets this to true
                        }),
...
mod tests {
    ...
    fn test_read_write_set_tracking() {
        ...
        if tx.value > 0 {
            assert!(
                dep.read_set.addresses.contains(&tx.to) || dep.write_set.addresses.contains(&tx.to)
            );
        }
    }
    ...
    fn test_execute_parallel_sync_disabled() {
        ...
        let results = exec.execute_parallel_sync(transactions.clone(), &|_tx: &Transaction| {
    ...
    fn test_execute_parallel_sync_single_transaction() {
        let executor = ParallelEvmExecutor::new();
        let transactions = vec![Transaction::with_data(
            Address::from([1; 20]),
            Address::from([2; 20]),
            1000,
            100,
        ...
        let results = executor.execute_parallel_sync(transactions, &|_tx: &Transaction| {
            Ok(crate::evm::ExecutionResult {
                success: true,
                gas_used: 21_000,
                output: Vec::new(),
    ...
    fn test_complex_dependency_chain() {
        ...
        let tx2 = Transaction::with_data(
            ...
            Vec::new(),
            21_000,
        ); // Depends on tx1 (reads from tx1's recipient)
        let tx3 = Transaction::with_data(
            Address::from([3; 20]),
            Address::from([4; 20]),
            3000,
            100,
    ...
    /// TST-005: Real parallel EVM execution test
    /// Tests conflict detection, batching logic, and executed_in_parallel flag behavior
    #[test]
    fn test_real_parallel_execution() {
        let executor = ParallelEvmExecutor::new();

        // 1. Create 2+ non-conflicting transactions (different 'to' addresses, different data)
        let tx1 = Transaction::with_data(
            Address::from([1; 20]), // sender
            Address::from([2; 20]), // recipient A
            1000,                   // value
            100,                    // nonce
            0,                      // gas_price
            vec![0xAA, 0xBB, 0xCC], // unique data for tx1
            21_000,
        );
        let tx2 = Transaction::with_data(
            Address::from([3; 20]), // sender (different from tx1)
            Address::from([4; 20]), // recipient B (different from tx1)
            2000,                   // value
            100,                    // nonce
            0,                      // gas_price
            vec![0xDD, 0xEE, 0xFF], // unique data for tx2
            21_000,
        );
        let tx3 = Transaction::with_data(
            Address::from([5; 20]), // sender (different from tx1, tx2)
            Address::from([6; 20]), // recipient C (different from tx1, tx2)
            3000,                   // value
            100,                    // nonce
            0,                      // gas_price
            vec![0x11, 0x22, 0x33], // unique data for tx3
            21_000,
        );

        // Track execution order to verify batching behavior
        let execution_order = std::sync::Arc::new(std::sync::Mutex::new(Vec::new()));
        let order_clone = execution_order.clone();

        // 2. Call execute_parallel_sync() with these transactions
        ...
        // 3. Verify both transactions were executed (check results length)
        assert!(results.is_ok(), "execute_parallel_sync should succeed");
        let results = results.unwrap();
        assert_eq!(results.len(), 3, "All 3 transactions should be executed");

        // 4. Verify executed_in_parallel flag behavior
        // Note: execute_parallel_sync sets executed_in_parallel = false because it executes sequentially
        // The async version (execute_parallel_async) would set it to true for batched transactions
        for result in &results {
            assert!(result.success, "Each transaction should succeed");
            // Sync version always sets executed_in_parallel = false
            // This verifies the flag behavior is correct
            assert!(
                !result.executed_in_parallel,
                "Sync executor should set executed_in_parallel = false"
            );
        }

        // Verify the output data matches what we sent (proves real execution)
        assert_eq!(
            results[0].output,
            vec![0xAA, 0xBB, 0xCC],
            "tx1 output should match input data"
        );
        assert_eq!(
            results[1].output,
            vec![0xDD, 0xEE, 0xFF],
            "tx2 output should match input data"
        );
        assert_eq!(
            results[2].output,
            vec![0x11, 0x22, 0x33],
            "tx3 output should match input data"
        );

        // Verify conflict detection: these transactions should NOT conflict
        // because they have different senders and different recipients
        let mut graph = DependencyGraph::new(vec![tx1.clone(), tx2.clone(), tx3.clone()]);
        assert!(
            graph.analyze().is_ok(),
            "Dependency analysis should succeed"
        );

        let batches = graph.group_parallel_batches();
        // Non-conflicting transactions can be batched together
        assert!(
            batches.len() <= 2,
            "Non-conflicting transactions should batch efficiently, got {} batches",
            batches.len()
        );

        // Verify all transactions are accounted for in batches
        let total_in_batches: usize = batches.iter().map(|b| b.len()).sum();
        assert_eq!(
            total_in_batches, 3,
            "All 3 transactions should be in batches"
        );
    }
}
...
mod integration_tests {
    ...
    fn test_async_execution_basic() {
        ...
        let executor_arc = Arc::new(move |_tx: Transaction| async move {
            Ok(crate::evm::ExecutionResult {
                success: true,
                gas_used: 21_000,
                output: Vec::new(),
    ...
    fn test_async_execution_with_errors() {
        ...
        let executor_arc =
            Arc::new(move |_tx: Transaction| async move { Err("Execution failed".to_string()) });
