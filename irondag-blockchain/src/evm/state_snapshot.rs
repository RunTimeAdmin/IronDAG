...
//!
//! Provides snapshot isolation for EVM transaction execution, enabling safe parallel
//! processing without cross-transaction interference. Each snapshot captures a point-in-time
//! view of blockchain state (account balances, nonces, block context).
//!
//! # Snapshot Lifecycle
//!
//! 1. **Creation**: A base snapshot is created from current state before parallel execution
//! 2. **Branching**: Independent branch snapshots are derived from the base for each parallel
//!    execution context, ensuring complete isolation
//! 3. **Modification**: Each branch applies transactions to its own snapshot without affecting
//!    others or the base state
//! 4. **Merge**: After parallel execution completes, branch snapshots are merged back using
//!    conflict resolution (takes maximum values for balances/nonces)
//!
//! # Usage
//!
//! ```ignore
...
//! let branch = manager.create_branch_snapshot(1)?;
//! // ... execute transactions on branch snapshot ...
//! let merged = manager.merge_all_branches()?;
//! ```
//!
//! # Related Modules
//!
//! - [`parallel.rs`]: The parallel execution engine that consumes snapshots
//! - [`benchmark.rs`]: Performance benchmarks for parallel execution (snapshots used indirectly
...
impl StateSnapshot {
    ...
    pub fn apply_transaction(&mut self, tx: &Transaction) -> Result<(), String> {
        ...
        self.set_nonce(tx.from, current_nonce.saturating_add(1));

        // Add to recipient (if not zero address)
        if tx.to != Address::zero() {
            let new_to_balance = self.get_balance(&tx.to).saturating_add(tx.value);
