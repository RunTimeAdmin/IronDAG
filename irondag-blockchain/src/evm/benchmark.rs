...
impl ParallelEvmBenchmark {
    ...
    pub fn benchmark(
        ...
        let _sequential_results: Vec<_> = transactions.iter().map(|tx| executor_fn(tx)).collect();
...
mod tests {
    ...
    fn test_generate_test_transactions() {
        ...
        for (i, item) in transactions.iter().enumerate().take(5) {
            assert_eq!(item.from[0], i as u8);
        }
    }
}
