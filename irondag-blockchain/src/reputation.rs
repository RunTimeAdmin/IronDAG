...
pub struct ReputationManager {
    ...
    /// Track unique contacts per address (address -> set of contacted addresses)
    seen_contacts: HashMap<Address, HashSet<Address>>,
}

impl ReputationManager {
    pub fn new() -> Self {
        Self::with_optional_registry(None)
    }

    pub fn with_node_registry(node_registry: Arc<RwLock<NodeRegistry>>) -> Self {
        Self::with_optional_registry(Some(node_registry))
    }

    /// Consolidated initialization method
    fn with_optional_registry(node_registry: Option<Arc<RwLock<NodeRegistry>>>) -> Self {
        Self {
            reputations: HashMap::new(),
            factors: HashMap::new(),
            node_registry,
            seen_contacts: HashMap::new(),
        }
    }
    ...
    pub fn calculate_reputation(&mut self, address: &Address) -> ReputationScore {
        let factors = self
            .factors
            .entry(*address)
            .or_insert_with(|| ReputationFactors {
                successful_txs: 0,
                failed_txs: 0,
                blocks_mined: 0,
                node_longevity: None,
    ...
    pub fn record_successful_tx(&mut self, address: &Address, value: u128, to: &Address) {
        ...
        // Track unique contacts using HashSet to avoid double-counting
        if *to != Address::zero() {
            let seen = self
                .seen_contacts
                .entry(*address)
            ...
            if seen.insert(*to) {
                // Only increment on first contact with this address
    ...
    pub async fn update_node_longevity(
        &mut self,
        address: &Address,
        node_identity: &crate::governance::node_identity::NodeIdentity,
    ) {
