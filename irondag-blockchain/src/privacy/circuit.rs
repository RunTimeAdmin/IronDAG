...
impl ConstraintSynthesizer<Fr> for PrivateTransferCircuit {
    fn generate_constraints(self, cs: ConstraintSystemRef<Fr>) -> Result<(), SynthesisError> {
        ...
        // arkworks 0.4: LinearCombination uses += operator with (coeff, variable) tuples
        let one = Fr::from(1u64);
        let mut a = ark_relations::r1cs::LinearCombination::<Fr>::zero();
        a += (one, old_balance_var);
        let mut b = ark_relations::r1cs::LinearCombination::<Fr>::zero();
        b += (one, Variable::One);
        let mut c = ark_relations::r1cs::LinearCombination::<Fr>::zero();
        c += (one, amount_var);
        c += (one, new_balance_var);
...
impl PrivacyCircuit for PrivateTransferCircuit {
    fn public_inputs(&self) -> Vec<Fr> {
        vec![
            Fr::from(self.new_balance.unwrap_or(0u128)),
            self.nullifier,
