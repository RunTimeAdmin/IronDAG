...
mod end_to_end_tests {
    ...
    use crate::privacy::{
        generate_keys, Nullifier, PedersenCommitment, PrivacyManager, PrivacyProver,
        PrivacyTransaction, PrivacyTxType, PrivacyVerifier, PrivateTransferCircuit,
    };
    use ark_bn254::Fr;
    ...
    fn test_complete_private_transfer_flow() {
        ...
        for input in &public_inputs {
            public_inputs_bytes.push(input.into_bigint().to_bytes_le());
        }
    ...
    fn test_double_spend_prevention() {
        ...
        rt.block_on(async {
            ...
            assert!(
                result2.unwrap_err().contains("already spent"),
    ...
    fn test_privacy_manager_enabled_disabled() {
        ...
        rt.block_on(async {
            ...
            assert!(
                privacy_manager_enabled.is_enabled(),
                "Privacy should be enabled"
            );

            // Test with privacy disabled
            let privacy_manager_disabled = PrivacyManager::new(false);
            assert!(
                !privacy_manager_disabled.is_enabled(),
                "Privacy should be disabled"
            );

            // Try to process transaction with disabled privacy
            let privacy_tx =
                PrivacyTransaction::new(PrivacyTxType::PrivateTransfer, vec![], vec![]);

            let result = privacy_manager_disabled
                .process_transaction(&privacy_tx)
                .await;
            assert!(
                result.is_err(),
                "Transaction should fail when privacy is disabled"
            );
            assert!(
