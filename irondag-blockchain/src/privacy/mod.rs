...
pub use circuit::{PrivacyCircuit, PrivateTransferCircuit};
pub use commitment::{Commitment, PedersenCommitment};
pub use keys::{
    derive_pedersen_generators, generate_keys, get_pedersen_commitment, load_keys_from_bytes,
    load_keys_from_paths, serialize_keys,
};
...
pub struct PrivacyConfig {
    ...
    /// Path to proving key from trusted setup (if set, load instead of generating per-node)
    #[serde(default)]
    pub proving_key_path: Option<std::path::PathBuf>,
    /// Path to verifying key from trusted setup (if set, load instead of generating per-node)
    #[serde(default)]
    pub verifying_key_path: Option<std::path::PathBuf>,
}
...
impl PrivacyNote {
    ...
    pub fn new(amount: u128, receiver: crate::types::Address, blinding: [u8; 32]) -> Self {
