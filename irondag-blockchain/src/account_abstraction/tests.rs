...
mod tests {
    ...
    async fn test_basic_wallet_creation() {
        ...
        let wallet =
            WalletFactory::create_basic_wallet(owner, salt).expect("Failed to create basic wallet");
    ...
    async fn test_multisig_wallet_creation() {
        ...
        let wallet =
            WalletFactory::create_multisig_wallet(owner, salt, signers.clone(), threshold).unwrap();
    ...
    async fn test_spending_limit_wallet_creation() {
        ...
        .expect("Failed to create spending limit wallet");
    ...
    async fn test_wallet_address_derivation() {
        ...
        let wallet3 = WalletFactory::create_basic_wallet(owner, 12346)
            .expect("Failed to create basic wallet");
        assert_ne!(wallet1.address, wallet3.address);

        // Different type = different address
        let wallet4 =
            WalletFactory::create_multisig_wallet(owner, salt, vec![[2u8; 20]], 1).unwrap();
    ...
    async fn test_wallet_nonce_management() {
        ...
        let owner = [1u8; 20];
        let wallet =
            WalletFactory::create_basic_wallet(owner, 0).expect("Failed to create basic wallet");
    ...
    async fn test_owner_wallet_tracking() {
        ...
        let wallet1 =
            WalletFactory::create_basic_wallet(owner, 0).expect("Failed to create basic wallet");
        let wallet2 =
            WalletFactory::create_basic_wallet(owner, 1).expect("Failed to create basic wallet");
        let wallet3 = WalletFactory::create_multisig_wallet(owner, 0, vec![[2u8; 20]], 1).unwrap();
    ...
    async fn test_multisig_threshold_validation() {
        ...
        assert!(
            WalletFactory::create_multisig_wallet(owner, 0, vec![[2u8; 20], [3u8; 20]], 5).is_err()
        );

        // Invalid: threshold zero
        assert!(
            WalletFactory::create_multisig_wallet(owner, 0, vec![[2u8; 20], [3u8; 20]], 0).is_err()
        );

        // Invalid: empty signers
        assert!(WalletFactory::create_multisig_wallet(owner, 0, vec![], 1).is_err());
    }
    ...
    async fn test_social_recovery_threshold_validation() {
        ...
        assert!(WalletFactory::create_social_recovery_wallet(
            owner,
            0,
            vec![[2u8; 20], [3u8; 20], [4u8; 20], [5u8; 20]],
            3,
            604800
        )
        .is_ok());
        ...
        assert!(WalletFactory::create_social_recovery_wallet(owner, 0, vec![], 1, 604800).is_err());
    }
}
