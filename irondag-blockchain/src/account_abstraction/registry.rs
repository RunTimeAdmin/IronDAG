...
pub struct WalletRegistry {
    ...
    owner_wallets: HashMap<[u8; 20], Vec<Address>>,
}
...
impl WalletRegistry {
    ...
    pub fn get_owner_wallets(&self, owner: &[u8; 20]) -> Vec<&SmartContractWallet> {
        self.owner_wallets
            .get(owner)
            .map(|addresses| {
                addresses
...
mod tests {
    ...
    fn test_wallet_registry() {
        ...
        let owner = [1u8; 20];
        let wallet = SmartContractWallet::new_basic(Address([2u8; 20]), owner)
