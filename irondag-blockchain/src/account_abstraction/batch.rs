...
impl BatchManager {
    ...
    pub fn cleanup(&mut self) {
        self.batches.retain(|_, batch| {
            matches!(batch.status, BatchStatus::Pending | BatchStatus::Executing)
        });
    }

    /// Execute a batch transaction atomically.
    /// On failure (or gas exceeded), calls `revert_fn` for each completed operation index in reverse order to roll back state.
    /// Returns results and total gas used.
    pub fn execute_batch(
        &mut self,
        batch_id: &Hash,
        execute_fn: impl Fn(&BatchOperation, usize) -> Result<BatchOperationResult, String>,
        revert_fn: impl Fn(usize, &BatchOperationResult),
                ...
                Ok(result) => {
                    ...
                    if total_gas > batch.gas_limit {
                        ...
                        for (i, r) in results.iter().enumerate().rev() {
                            if i < results.len() && batch.operations.get(i).is_some() {
                                revert_fn(i, r);
                            }
                        }
                        batch.mark_failed(results, total_gas);
                        return Err("Batch gas limit exceeded".to_string());
                    }
                    results.push(result.clone());

                    if !result.success {
                        for (i, r) in results.iter().enumerate().rev() {
                            if batch.operations.get(i).is_some() {
                                revert_fn(i, r);
                            }
                        }
                        batch.mark_failed(results, total_gas);
    ...
    pub fn estimate_gas(&self, operations: &[BatchOperation]) -> Result<GasEstimate, String> {
        if operations.is_empty() {
...
mod tests {
    ...
    fn test_batch_validation() {
        ...
        let batch = BatchTransaction::new(wallet, operations, 1, 100_000, 1_000_000_000, timestamp);
    ...
    fn test_batch_status_transitions() {
        ...
        let mut batch =
            BatchTransaction::new(wallet, operations, 1, 100_000, 1_000_000_000, timestamp);
