...
mod tests {
    ...
    async fn test_e2e_multisig_workflow() {
...

        {
            ...
            let mut tx = crate::blockchain::Transaction::new(wallet.address, to, value, 0, 0);
            tx.hash = crate::types::Hash(tx_hash);
            let multisig_tx =
        ...
        // Verify pending transaction exists
        {
            ...
            assert_eq!(pending[0].transaction.hash.0, tx_hash);
        }
    }
    ...
    async fn test_e2e_combined_wallet_features() {
        ...
        let owner = [1u8; 20];
        let guardians = vec![[4u8; 20], [5u8; 20]];

        // Create social recovery wallet
        let wallet =
            WalletFactory::create_social_recovery_wallet(owner, 0, guardians.clone(), 2, 0)
                .unwrap();
