...
use crate::account_abstraction::wallet::{
    SmartContractWallet, SpendingLimits, WalletConfig, WalletType,
};
...
impl WalletFactory {
    ...
    pub fn derive_wallet_address(owner: [u8; 20], salt: u64, wallet_type: &WalletType) -> Address {
    ...
    pub fn create_basic_wallet(owner: [u8; 20], salt: u64) -> Result<SmartContractWallet, String> {
    ...
    pub fn create_social_recovery_wallet(
        ...
        let address = Self::derive_wallet_address(owner, salt, &wallet_type);
        SmartContractWallet::new_social_recovery(
            address,
            owner,
            guardians,
            recovery_threshold,
    ...
    pub fn create_spending_limit_wallet(
        ...
        let limits = SpendingLimits::new(daily_limit, weekly_limit, monthly_limit)?;
        let wallet_type = WalletType::SpendingLimit {
            limits: limits.clone(),
        };
        let address = Self::derive_wallet_address(owner, salt, &wallet_type);

        Ok(SmartContractWallet {
            address,
            owner,
            wallet_type,
            config: WalletConfig {
...
mod tests {
    ...
    fn test_wallet_address_derivation() {
        ...
        let address4 = WalletFactory::derive_wallet_address(
            owner,
            salt,
            &WalletType::MultiSig {
                signers: vec![],
