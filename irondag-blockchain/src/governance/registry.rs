...
impl NodeRegistry {
    ...
    pub fn register_node(&mut self, identity: NodeIdentity) -> Result<(), String> {
        ...
        // Initialize stats
        self.node_stats
            .insert(identity.clone(), NodeLongevity::new(identity));
...
impl LongevityTracker {
    ...
    pub fn new(registry: Arc<RwLock<NodeRegistry>>, blockchain: Arc<RwLock<Blockchain>>) -> Self {
        Self {
    ...
    async fn get_participation_records(
        &self,
        _today: u64,
    ) -> HashMap<NodeIdentity, ParticipationRecord> {
