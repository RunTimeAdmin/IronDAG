...
impl HardwareFingerprint {
    ...
    pub fn generate(node_private_key: &[u8; 32]) -> Self {
        ...
        hasher.update(cpu_model_hash.as_ref());
        hasher.update(bios_serial_hash.as_ref());
        hasher.update(mac_address_hash.as_ref());
        hasher.update(system_uuid_hash.as_ref());
        if let Some(ref puf) = puf_id {
            hasher.update(puf.as_ref());
        }
        let fingerprint: Hash = Hash(hasher.finalize().into());
...
impl ZkUniquenessProof {
    ...
    pub fn generate(hardware_fingerprint: &HardwareFingerprint) -> Result<Self, String> {
        ...
        let zk_proof =
            generate_zk_proof_placeholder(&hardware_fingerprint.fingerprint, &commitment);
...
fn sign_fingerprint(fingerprint: &Hash, private_key: &[u8; 32]) -> Vec<u8> {
    ...
    let mut hasher = Keccak256::new();
    hasher.update(fingerprint.as_ref());
...
fn verify_signature(fingerprint: &Hash, signature: &[u8], public_key: &[u8; 32]) -> bool {
    ...
    signature == AsRef::<[u8]>::as_ref(&expected)
}
