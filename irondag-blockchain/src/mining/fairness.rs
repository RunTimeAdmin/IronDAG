...
impl FairnessAnalyzer {
    ...
    pub fn analyze_block(&self, block: &Block) -> FairnessMetrics {
        ...
        // Detect MEV patterns
        let (sandwich, backrun, frontrun, mev_value) =
            self.detect_mev_patterns(&block.transactions);
        ...
        let fairness_score =
            (1.0 - reordering_penalty * 0.4 - mev_penalty - concentration_penalty).max(0.0);
    ...
    fn detect_mev_patterns(&self, transactions: &[Transaction]) -> (u64, u64, u64, u128) {
        ...
        for i in 0..transactions.len().saturating_sub(2) {
            ...
            if tx1.to == tx3.to && tx1.to != Address::zero() && tx2.to == tx1.to {
        ...
        for i in 0..transactions.len().saturating_sub(1) {
            ...
            if tx1.to == tx2.to && tx1.to != Address::zero() && tx1.from != tx2.from {
    ...
    pub fn get_transaction_fairness(
        &self,
        tx_hash: &Hash,
        position: usize,
        total_txs: usize,
    ) -> f64 {
        if let Some(&_arrival_time) = self.transaction_arrival_times.get(tx_hash) {
    ...
    pub fn clear_old_data(&mut self, before_timestamp: u64) {
        self.transaction_arrival_times
            .retain(|_, &mut time| time >= before_timestamp);
...
mod tests {
    ...
    fn test_empty_block_fairness() {
        ...
        let block = Block {
            header: BlockHeader::new(vec![], 0, StreamType::StreamA, 100, 1_000_000_000),
            transactions: vec![],
            hash: Hash([0u8; 32]),
            zk_proof: None,
        };
    ...
    fn test_sandwich_detection() {
        ...
        let target = [99u8; 20];
        let tx1 = create_test_transaction(Hash([1u8; 32]), [10u8; 20], target);
        let tx2 = create_test_transaction(Hash([2u8; 32]), [20u8; 20], target); // Victim
        let tx3 = create_test_transaction(Hash([3u8; 32]), [10u8; 20], target);
