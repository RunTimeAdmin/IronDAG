...
pub fn order_transactions(
    ...
    match policy {
        ...
        OrderingPolicy::FeeBased => {
            let mut txs = transactions;
            // EIP-1559: Sort by effective priority fee (descending), then by value (descending)
            // For now, we use the legacy fee field as a proxy for effective priority fee
            // since base_fee is not available in this context
            txs.sort_by(|a, b| {
                // For EIP-1559 transactions, use max_priority_fee_per_gas if available
                // Otherwise fall back to legacy fee calculation
                let fee_a = a.max_priority_fee_per_gas.unwrap_or(a.fee);
                let fee_b = b.max_priority_fee_per_gas.unwrap_or(b.fee);
        ...
        OrderingPolicy::Hybrid => {
            ...
            // EIP-1559: Sort by: priority fee score (weighted) + age bonus
            // High priority fee transactions get priority, but old transactions get a boost

            // Use max_priority_fee_per_gas if available, otherwise fall back to fee
            let max_fee = txs
                .iter()
                .map(|t| t.max_priority_fee_per_gas.unwrap_or(t.fee))
                .max()
                .unwrap_or(1) as f64;
            ...
            txs.sort_by(|a, b| {
                ...
                // Priority fee score (normalized to 0-1)
                let fee_a = a.max_priority_fee_per_gas.unwrap_or(a.fee) as f64;
                let fee_b = b.max_priority_fee_per_gas.unwrap_or(b.fee) as f64;
                let fee_score_a = (fee_a / max_fee) * 0.7;
                let fee_score_b = (fee_b / max_fee) * 0.7;
        ...
        OrderingPolicy::TimeWeighted => {
            let mut txs = transactions;
            // EIP-1559: Sort by: age (primary) + priority fee boost (secondary)
            // Older transactions get priority, but high priority fees can boost position
            txs.sort_by(|a, b| {
                ...
                match age_b.cmp(&age_a) {
                    // If ages are similar (within 5 seconds), use priority fee as tiebreaker
                    std::cmp::Ordering::Equal if age_a.abs_diff(age_b) <= 5 => fee_b.cmp(&fee_a),
                    other => other,
                }
            });
            txs
...
mod tests {
    ...
    fn create_tx(fee: u128, value: u128) -> Transaction {
        Transaction::new([0u8; 20], [0u8; 20], value, fee, 0)
    }

    #[test]
    fn test_fifo_ordering() {
        let mut context = OrderingContext::new();
        let t1 = create_tx(100, 1000);
        let t2 = create_tx(200, 2000);
        let t3 = create_tx(50, 500);
