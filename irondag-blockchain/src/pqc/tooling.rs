...
pub fn generate_pq_account(algorithm: &str) -> Result<PqAccount, String> {
    match algorithm.to_lowercase().as_str() {
        ...
        "sphincsplus" | "sphincs" => PqAccount::new_sphincsplus(),
        _ => Err(format!(
            "Unknown PQ algorithm: {}. Use 'dilithium3' or 'sphincsplus'",
            algorithm
        )),
    }
}
...
impl PqAccountExport {
    ...
    pub fn from_hex(hex_str: &str) -> Result<Self, String> {
        ...
        bincode::deserialize(&bytes).map_err(|e| format!("Invalid account data: {}", e))
    }
}
