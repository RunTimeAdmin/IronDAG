...
mod tests {
    ...
    use crate::stop_loss::{StopLossManager, StopLossStatus, StopLossType};
    ...
    async fn test_stop_loss_trigger() {
        ...
        manager
            .write()
            .await
            .set_initial_price(&order.stop_loss_id, 60_000_000_000_000_000_000)
            .unwrap();
        ...
        manager
            .write()
            .await
            .mark_triggered(&order.stop_loss_id, current_price, current_time)
            .unwrap();

        let manager_read = manager.read().await;
        let updated_order = manager_read.get(&order.stop_loss_id).unwrap();
