...
pub struct StopLossOrder {
    ...
    pub asset_pair: String, // e.g., "BTC/USD", "ETH/IDAG"
...
impl StopLossOrder {
    pub fn new(
        ...
        let stop_loss_id =
            Self::calculate_id(wallet_address, &asset_pair, &trigger_type, created_at);
    ...
    pub fn should_trigger(&self, current_price: u128, current_time: u64) -> bool {
        ...
        match &self.trigger_type {
            ...
            StopLossType::PercentChangeUp(percent) => {
                let change =
                    ((current_price as f64 - initial_price as f64) / initial_price as f64) * 100.0;
                change >= *percent
            }
            StopLossType::PercentChangeDown(percent) => {
                let change =
                    ((initial_price as f64 - current_price as f64) / initial_price as f64) * 100.0;
                change >= *percent
            }
            StopLossType::PriceRange { min, max } => current_price < *min || current_price > *max,
        }
    }
