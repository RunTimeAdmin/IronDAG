...
impl OracleNode {
    ...
    pub fn report_accuracy(&mut self, is_accurate: bool) {
        ...
        self.reputation_score = (self.accuracy_rate * 100.0).clamp(0.0, 100.0);
    }
...
impl OracleRegistry {
    ...
    pub fn report_oracle_accuracy(
        &mut self,
        address: &Address,
        is_accurate: bool,
    ) -> Result<(), String> {
        let oracle = self.nodes.get_mut(address).ok_or("Oracle not found")?;
...
mod tests {
    ...
    fn test_oracle_registration() {
        ...
        let stake = 2_000_000_000_000_000_000; // 2 IDAG

        assert!(registry
            .register_oracle(address, feed_types.clone(), stake, 1000)
            .is_ok());
    ...
    fn test_oracle_minimum_stake() {
        let mut registry = OracleRegistry::default();
        let address = Address::from([1; 20]);
        let stake = 500_000_000_000_000_000; // 0.5 IDAG (below minimum)

        assert!(registry
            .register_oracle(address, vec![FeedType::Price], stake, 1000)
            .is_err());
    }
