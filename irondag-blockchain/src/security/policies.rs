...
impl SecurityPolicyManager {
    ...
    fn policy_matches(
        ...
        match &policy.policy_type {
            PolicyType::MaxRiskScore { threshold } => risk_score.score > *threshold,
            ...
            PolicyType::MinConfidence { threshold } => risk_score.confidence < *threshold,
            PolicyType::Custom { .. } => {
...
mod tests {
    ...
    fn test_max_risk_score_policy() {
        ...
        let tx = Transaction::new(Address([1u8; 20]), Address([2u8; 20]), 1000, 100, 0);
        let evaluation = manager.evaluate_transaction(&tx, &risk_score, Address([1u8; 20]));
    ...
    fn test_block_address_policy() {
        ...
        let blocked_address = Address([9u8; 20]);
        ...
        let tx = Transaction::new(Address([1u8; 20]), blocked_address, 1000, 100, 0);
        let evaluation = manager.evaluate_transaction(&tx, &risk_score, Address([1u8; 20]));
