    ...
    fn matches(&self, tx: &Transaction) -> bool {
        match self {
            PatternRule::HoneypotPattern => {
                ...
                tx.to == Address::zero() && tx.value < 1000 && !tx.data.is_empty()
            }
            PatternRule::MixerPattern => {
...
mod tests {
    ...
    fn create_test_transaction(
        ...
        Transaction::with_data(from, to, value, 1000, 0, data, 21_000)
    }
    ...
    fn test_honeypot_detection() {
        ...
        let tx = create_test_transaction(
            Address([1u8; 20]),
            Address([0u8; 20]), // Contract deployment
            100,                // Small value
            vec![1, 2, 3],      // Has data
        );
    ...
    fn test_malicious_address_detection() {
        ...
        let malicious_addr = Address([99u8; 20]);
        detector.add_malicious_address(malicious_addr, "Known scammer".to_string());

        let tx = create_test_transaction(malicious_addr, Address([2u8; 20]), 1000, vec![]);

        let analysis = detector.analyze_transaction(&tx);
        assert!(analysis.risk_score >= 0.8);
        assert!(analysis
            .labels
            .contains(&"known_malicious_address".to_string()));
    }
}
