# IronDAG Blockchain v1.0 Specification
...
This document defines the complete specification for IronDAG Blockchain v1.0, including protocol parameters, API surface, network protocol, consensus rules, and storage format.
...
BLOCK_REWARD: 50 IronDAG tokens (50_000_000_000_000_000_000 base units)
MINING_ALGORITHM: Blake3
```

#### Stream B (CPU - KHeavyHash; GPU planned)
```rust
BLOCK_TIME: 1 second
MAX_TRANSACTIONS: 5,000
BLOCK_REWARD: 25 IronDAG tokens (25_000_000_000_000_000_000 base units)
...
BLOCK_REWARD: 0 IronDAG tokens (fee-based only)
...
```rust
CHAIN_ID: 0x50595258 // "IronDAG" in ASCII hex
...
- **Fixed Fee**: 0.001 IronDAG per transaction (if EVM disabled)
...
- **Development Team**: 10% (1,000,000,000 IronDAG)
- **Community Fund**: 5% (500,000,000 IronDAG)
- **Testnet Faucet**: 1% (100,000,000 IronDAG)
- **Reserved**: 84% (8,400,000,000 IronDAG)
...
- **Block Time**: 1s (Stream C), 5s (Stream B), 10s (Stream A)
- **Finality**: 1s (Stream C with ZK proofs)
